...
subroutine functs(n,z,M,f)
	implicit none
	integer	:: n, M, i, j, ii, jj, ind
	real*8		:: z(n), f(M)
	real*8		:: pi, pi2
	real*8		:: t1(8), t2(8), t3(3), x(3), h(3)
	real*8		:: S(3), Av(2), zmax(8), y(8), w(8), r_sum(8)
!	real*8		:: t1(n), t2(n), t3(M), x(M), h(M)
!	real*8		:: S(M), Av(M-1), zmax(n), y(n), w(n), r_sum(n)
	real*8		:: a, b, asum
...
!    else y[i]^(BB+(CC-BB)*(AA-(1-2*r_sum[i])*abs(floor(0.5-r_sum[i])+AA)));
	...
	do i = 2,n
		t1(i) = y(i)**(BB+(CC-BB)*	&
			(AA-(1.d0-2.d0*r_sum(i))*abs(floor(0.5d0-r_sum(i))+AA)))
	enddo
!# second level mapping
!var t2{i in 1..n} = if i<=k then t1[i]
!    else abs(t1[i]-0.35)/abs(floor(0.35-t1[i])+0.35);
...
!    else sum {ii in k+1..n} (t2[ii]+sum {jj in 0..(l-2)} abs(t2[ii]-t2[k+1+((ii+jj-(k+1)+1) mod (n-k))]))/(((n-k)/l)*ceil(l/2)*(1+2*l-2*ceil(l/2)));
	do i = 1,M-1
		...
		do ii = ((i-1)*k/(M-1)+1), (i*k/(M-1))
			asum = 0.d0
			do jj = 0,(k/(M-1)-2)
				ind = ((i-1)*k/(M-1)+1)+mod( (ii+jj-((i-1)*k/(M-1)+1)+1), &
					   ((i*k/(M-1))-((i-1)*k/(M-1)+1)+1) )
				asum = asum + abs(t2(ii)-t2(ind))
			enddo
			a = a+ t2(ii) + asum
		enddo
		t3(i) = a/(((i*kr/dble(M-1))-((i-1)*kr/dble(M-1)+1.d0)+1.d0)/	&
			(kr/dble(M-1))*ceiling(kr/dble(M-1)/2.d0)*(1.d0+2.d0*kr/	&
		 	dble(M-1)-2.d0*ceiling(kr/dble(M-1)/2.d0)))
	enddo
	a = 0.d0
	b = 0.d0
	do ii = k+1,n
		asum = 0.d0
		do jj = 0,l-2
			asum = asum + abs(t2(ii)-t2(k+1+mod((ii+jj-(k+1)+1),(n-k))))
		enddo
		a = a + t2(ii) + asum
	enddo
	t3(M) = a/((dble(n-k)/lr)*ceiling(lr/2.d0)*(1.d0+2.d0*lr-2.d0*ceiling(lr/2.d0)))
